...
interface UserProfile {
  ...
  email: string;
  userRole: string;  // Changed from userType
  firstName: string;
  middleName: string;
  lastName: string;
  ...
  city: string;
  county: string;
  zipCode: string;  // Changed from zipcode
  address: string;  // Changed from streetAddress
  mapLink: string;
  ...
  faxNumber: string;  // Changed from faxNo
  linkedinID: string;  // Changed from linkedinId
  facebookID: string;  // Changed from facebookId
  instagramID: string;  // Changed from instagramId
  twitterID: string;  // Changed from twitterId
}

const formSchema = z.object({
  email: z.string().email("Invalid email format"),
  userRole: z.string().min(1, "User role is required"),  // Changed from userType
  firstName: z.string().optional(),
  middleName: z.string().optional(),
  lastName: z.string().optional(),
  dob: z.string().optional(),  // Changed from dateOfBirth
  ...
  zipCode: z.string().optional(),  // Changed from zipcode
  address: z.string().optional(),  // Changed from streetAddress
  mapLink: z.string().regex(urlRegex, "Invalid URL format").optional().or(z.literal("")),
  divisionalGroup: z.string().optional(),
  division: z.string().optional(),
  subdivision: z.string().optional(),
  sector: z.string().optional(),
  phone: z.string().regex(phoneRegex, "Invalid phone number format").optional().or(z.literal("")),  // Changed from phoneNo
  alternatePhone: z.string().regex(phoneRegex, "Invalid phone number format").optional().or(z.literal("")),
  faxNumber: z.string().optional(),  // Changed from faxNo
  linkedinID: z.string().regex(urlRegex, "Invalid URL format").optional().or(z.literal("")),  // Changed from linkedinId
  facebookID: z.string().regex(urlRegex, "Invalid URL format").optional().or(z.literal("")),  // Changed from facebookId
  instagramID: z.string().regex(urlRegex, "Invalid URL format").optional().or(z.literal("")),  // Changed from instagramId
  twitterID: z.string().regex(urlRegex, "Invalid URL format").optional().or(z.literal("")),  // Changed from twitterId
});
...
export default function EditInternalProfile() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const response = await fetch(`/api/WAVInternalUser/${params.id}`, {
        method: 'PUT',
        headers: {
      ...
      router.push(`/internal/dashboard/internal-profile/${params.id}`);
    } catch (error) {
    ...
    <div className="space-y-6 p-6 max-w-[1200px] mx-auto">
      ...
      <div className="flex items-center justify-between text-sm">
        <div className="flex items-center gap-1">
          ...
          <span className="text-muted-foreground">Edit Internal Profile</span>
        </div>
        <Button
          variant="outline"

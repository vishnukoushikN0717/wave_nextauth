...
function VerifyOTPContent() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validateResponse = await fetch("/api/WAVExternalUser/validate-otp", {
        method: "POST",
        headers: {
          accept: "*/*",
      ...
      if (!validateResponse.ok) {
        setMessage({
          text: validationData.message || "Invalid OTP",
          type: "error",
        })
        return
      }

      const userResponse = await fetch(`/api/WAVEXternalUser/byEmail/${email}`, {
        method: "GET",
        headers: {
          accept: "*/*",
        },
      ...
      const userDataToStore = {
        email: email,
        id: userData.id,
        wavInternalUserId: userData.wavInternalUserId,
        onboarderStatus: userData.onboarderStatus,
        userRole: userData.userRole
      }

      localStorage.setItem("user", JSON.stringify(userDataToStore))
      document.cookie = `auth=${JSON.stringify(userDataToStore)}; path=/; max-age=${30 * 24 * 60 * 60}; SameSite=Lax`

      if (!userData.onboarderStatus) {
        console.log("User not onboarded, showing modal")
